...
namespace Lean.Touch
{
	...
	public class LeanRotate : MonoBehaviour
	{
		...
		protected virtual void Translate(float twistDegrees, Vector2 twistScreenCenter)
		{
			...
			var camera2 = LeanTouch.GetCamera(Camera, gameObject);

			if (camera2 != null)
			{
				// Screen position of the transform
				var screenPoint = camera2.WorldToScreenPoint(transform.position);

				// Twist screen point around the twistScreenCenter by twistDegrees
				var twistRotation = Quaternion.Euler(0.0f, twistDegrees, 0.0f);
                ...
                // Convert back to world space
                transform.position = camera2.ScreenToWorldPoint(screenPoint);
			}
		}

		protected virtual void RotateUI(float twistDegrees)
		{
			transform.rotation *= Quaternion.Euler(0.0f, twistDegrees, 0.0f);
		}
		...
		protected virtual void Rotate(float twistDegrees)
		{
			...
			if (camera2 != null)
			{
                ...
                var axis = transform.InverseTransformDirection(camera2.transform.up);
				transform.rotation *= Quaternion.AngleAxis(twistDegrees, -eulerAngles);
            }
		}
	}
}